 ...
 *  instead of loading an image. Plain images (not video) are click-to-zoom via
 *  the lightbox; the caption stays outside the trigger. */
export default function MediaFrame({ media, sizes }: { media: CaseMedia; sizes: string }) {
  ...
  const alt = media.caption ? stripInline(media.caption) : "";
  ...
  return (
    <figure>
      {media.video ? (
        ...
        <ImageLightbox src={media.src} alt={alt} title={alt || media.kind}>
          {frame}
        </ImageLightbox>
      )}
      {media.caption && (
