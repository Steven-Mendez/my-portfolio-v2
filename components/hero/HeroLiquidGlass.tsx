    ...
    >
      <div className="w-full max-w-88 sm:max-w-2xl md:max-w-4xl animate-in slide-in-from-bottom-8 duration-700 fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2.5 sm:gap-3 mt-10 md:mt-20">
            <a 
              href="#experience" 
              className="w-full sm:w-auto justify-center flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3.5 md:py-3 text-[10px] md:text-xs tracking-[0.14em] md:tracking-[0.2em] font-bold hover:scale-[1.02] transition-all rounded-full group shadow-lg"
            >
                VIEW EXPERIENCE
            ...
            <a href="/resume.pdf" download={`${portfolioData.profile.fullName.replace(' ', '_')}_Resume.pdf`} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center rounded-full px-4 py-2.5 md:px-5 md:py-3 border border-white/20 bg-white/5 hover:bg-white/10 hover:border-white/40 text-white backdrop-blur-md transition-all shadow-lg text-xs font-medium group">
